                ...
                for f_id in range(len(T[src, dst]['Flows'])):
                    if T[src, dst]['Flows'][f_id]['AvgBw'] != 0 and T[src, dst]['Flows'][f_id]['PktsGen'] != 0:
                        D_G.add_node('p_{}_{}_{}'.format(src, dst, f_id),
                                     ...
                                     pathLength=len(R[src,dst]) #length of the path
                                     )
...
def hypergraph_to_input_data(hypergraph):
    ...
    return {"traffic": list(nx.get_node_attributes(D_G, 'traffic').values()),
            ...
            "pathLength": list(nx.get_node_attributes(D_G, 'pathLength').values()),
            ...
            "n_links": n_l,
            "n_paths": n_p
            }, list(nx.get_node_attributes(D_G, 'occupancy').values()) #list(nx.get_node_attributes(D_G, 'delay').values())
    ...
    ds = tf.data.Dataset.from_generator(generator,
                                        ...
                                        output_types=(
                                            {"traffic": tf.float32,
                                             ...
                                             "path_to_link": tf.int32, 
                                             "path_ids": tf.int32,
                                             "sequence_links": tf.int32, 
                                             "sequence_path": tf.int32,
                                             "n_links": tf.int32, 
                                             "n_paths": tf.int32},
                                            tf.float32),
                                        output_shapes=(
                                            {"traffic": tf.TensorShape([None]),
                                             ...
                                             "pathLength": tf.TensorShape([None]),
